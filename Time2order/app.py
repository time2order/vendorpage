...
def register():
    ...
    if request.method == 'POST':
        ...
        if password != repassword:
            ...
            db.add_to_yettoapprove(username, password, email, mobile, shop_name, shop_owner_name, map_link, category)
...
def list_products():
    ...
    vendor_username = session['vendor_username']
    products = db.get_products_by_vendor(vendor_username)  # Fetch products from the database
...
if __name__ == '__main__':
    app.run(debug=True)
# This is a simple Flask application that allows vendors to log in, register, and add products.